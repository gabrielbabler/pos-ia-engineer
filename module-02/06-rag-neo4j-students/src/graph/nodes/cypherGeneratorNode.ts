...
export function createCypherGeneratorNode(
  ...
  return async (state: GraphState): Promise<Partial<GraphState>> => {
    try {
      ...
      const systemPromt = await getSystemPrompt(schema, SALES_CONTEXT)
      const userPrompt = getUserPromptTemplate(targetQuestion)

      const { data, error } = await llmClient.generateStructured(systemPromt, userPrompt, CypherQuerySchema)

      if(error) {
        return {
          error: `Failed to generate query: ${error ?? 'Unkown error'}`,
        }
      }
